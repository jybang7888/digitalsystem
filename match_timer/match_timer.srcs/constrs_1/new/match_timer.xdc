set_property IOSTANDARD LVCMOS33 [get_ports {SEG_COM[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_COM[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_COM[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_COM[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_COM[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_COM[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_DATA[7]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_DATA[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_DATA[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_DATA[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_DATA[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_DATA[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_DATA[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_DATA[0]}]
set_property PACKAGE_PIN T1 [get_ports {SEG_COM[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_COM[7]}]
set_property IOSTANDARD LVCMOS33 [get_ports {SEG_COM[6]}]
set_property PACKAGE_PIN R4 [get_ports {SEG_COM[4]}]
set_property PACKAGE_PIN R3 [get_ports {SEG_COM[3]}]
set_property PACKAGE_PIN R2 [get_ports {SEG_COM[2]}]


set_property DRIVE 12 [get_ports {SEG_COM[7]}]

set_property PACKAGE_PIN T2 [get_ports {SEG_COM[7]}]
set_property PACKAGE_PIN R5 [get_ports {SEG_COM[5]}]
set_property PACKAGE_PIN P3 [get_ports {SEG_COM[1]}]
set_property PACKAGE_PIN P2 [get_ports {SEG_COM[0]}]
set_property PACKAGE_PIN P1 [get_ports {SEG_DATA[7]}]
set_property PACKAGE_PIN N5 [get_ports {SEG_DATA[6]}]
set_property PACKAGE_PIN N4 [get_ports {SEG_DATA[5]}]
set_property PACKAGE_PIN N3 [get_ports {SEG_DATA[4]}]
set_property PACKAGE_PIN N1 [get_ports {SEG_DATA[3]}]
set_property PACKAGE_PIN M5 [get_ports {SEG_DATA[2]}]
set_property PACKAGE_PIN M4 [get_ports {SEG_DATA[1]}]
set_property PACKAGE_PIN M3 [get_ports {SEG_DATA[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {val_o[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {val_o[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {val_o[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {val_o[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {val_t[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {val_t[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {val_t[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {val_t[0]}]
set_property PACKAGE_PIN AB3 [get_ports {val_o[3]}]
set_property PACKAGE_PIN AB4 [get_ports {val_o[2]}]
set_property PACKAGE_PIN Y4 [get_ports {val_o[1]}]
set_property PACKAGE_PIN Y5 [get_ports {val_o[0]}]
set_property PACKAGE_PIN W5 [get_ports {val_t[3]}]
set_property PACKAGE_PIN V6 [get_ports {val_t[2]}]
set_property PACKAGE_PIN AB5 [get_ports {val_t[1]}]
set_property PACKAGE_PIN AB6 [get_ports {val_t[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports N_Reset]
set_property IOSTANDARD LVCMOS33 [get_ports CLK]

